        ...
        for i in range(len(x_test)):
            ...
            self.x_vars.append(
                self.gurobi_model.addVar(
                    lb=float(t_i - perturbation),
                    ub=float(t_i + perturbation),
    ...
    def add_relu_layer(self, z, lower_bounds, upper_bounds):
        ...
        for i in range(n_inputs):
            ...
            if u_i <= 0:
                ...
                self.gurobi_model.addConstr(p_vars[i] == 0)
            elif l_i >= 0:
                # always active
                self.gurobi_model.addConstr(hatz_vars[i] == z[i])
                self.gurobi_model.addConstr(p_vars[i] == 1)
            else:
                # unstable
                n_unstable += 1
                self.gurobi_model.addConstr(hatz_vars[i] >= z[i])
                self.gurobi_model.addConstr(hatz_vars[i] >= 0)
                # hatz <= z - l*(1-p)
                self.gurobi_model.addConstr(hatz_vars[i] <= z[i] - l_i * (1 - p_vars[i]))
                # hatz <= u*p
                self.gurobi_model.addConstr(hatz_vars[i] <= u_i * p_vars[i])
